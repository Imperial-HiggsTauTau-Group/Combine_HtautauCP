...
for key in original_file.GetListOfKeys():
    if isinstance(original_file.Get(key.GetName()),ROOT.TDirectory):
        ...
        if (
            dirname in cp_bins and dirname.startswith('tt_')
            and not dirname.startswith("tt_mva_ggH")
            and not dirname.startswith("tt_mva_VBF")
            and not dirname.startswith("tt_tau_")
            and not dirname.startswith("tt_fake_")
        ):
           GetFFUncerts(dirname, original_file)
...
if (
    'tt_mva_ggH' in ff_aiso_yields
    and 'tt_mva_VBF' in ff_aiso_yields
    and 'tt_mva_tau' in ff_aiso_yields
    and 'tt_mva_fake' in ff_aiso_yields
):

    # now we estimate the normalization uncertainties for the FFs
    # get the correlated uncertainty from summing mva_ggH, mva_VBF, mva_tau, and mva_fake
    data_total = 0.
    fake_total = 0.
    for x in ['tt_mva_ggH', 'tt_mva_VBF', 'tt_mva_tau', 'tt_mva_fake']:
